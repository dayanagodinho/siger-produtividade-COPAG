...
function metaOuErro(valor, nome) {
  ...
  if (!Number.isFinite(n) || n < 0 || n > 60) return { erro: `${nome} precisa ser um número de horas entre 0 e 60 (recebido "${valor}")` };
  return { valor: n };
}
...
router.post('/', exigirChefia, async (req, res) => {
  ...
  if (!nome || !String(nome).trim() || !email) return res.status(400).json({ erro: 'Informe nome e login (e-mail ou usuário)' });
  if (!ehLogin(email)) return res.status(400).json({ erro: `Login inválido: "${email}". Use um e-mail ou um nome de usuário (letras, números, ponto), com 3 a 40 caracteres` });
  ...
  try {
    ...
    if (e.code === '23505') return res.status(409).json({ erro: 'Já existe servidor com esse email' });
    throw e;
  }
});
...
router.put('/:id', exigirChefia, async (req, res) => {
  ...
  if (email !== null && !ehLogin(email)) return res.status(400).json({ erro: `Login inválido: "${email}". Use um e-mail ou um nome de usuário (letras, números, ponto), com 3 a 40 caracteres` });
  ...
  if (proprio && (perfil === 'servidor' || ativo === false)) {
    return res.status(409).json({ erro: 'Você não pode tirar a própria chefia nem se tirar do acompanhamento. Peca a outra chefia' });
  }
  if (perfil === 'servidor' || ativo === false) {
    ...
    if (chefias.length === 1 && Number(chefias[0].id) === Number(req.params.id)) {
      return res.status(409).json({ erro: 'Essa é a única chefia ativa. Promova outra pessoa a chefia antes' });
    }
  }
  let rows;
  try {
  ...
  if (!rows[0]) return res.status(404).json({ erro: 'Servidor não encontrado' });
  res.json(rows[0]);
});
