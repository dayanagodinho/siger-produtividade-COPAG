...
rotasDeExportacao.get(
  ...
  rota(async (req, res) => {
    ...
    const { livro, aba } = prepararPlanilha('Lancamentos');
    aba.columns = [
      ...
      { header: 'Descricao', key: 'descricao', width: 44 },
      { header: 'Nivel', key: 'nivel', width: 8 },
      { header: 'Nivel declarado', key: 'nivel_original', width: 15 },
      ...
      { header: 'Conclusao', key: 'data_conclusao', width: 12 },
      { header: 'Status', key: 'status', width: 14 },
      { header: 'Validacao', key: 'situacao', width: 12 },
    ...
    for (const lancamento of lancamentos) {
      aba.addRow({
        ...
        status: lancamento.status === 'CONCLUIDO' ? 'Concluido' : 'Em andamento',
        nivel_original: lancamento.nivel_original ?? '',
      });
    }
    aba.getColumn('pontos').numFmt = '0.00';

    await enviar(res, livro, `lancamentos-${competencia.slice(0, 7)}.xlsx`);
  }),
);
...
rotasDeExportacao.get(
  ...
  rota(async (req, res) => {
    ...
    aba.columns = [
      ...
      { header: 'Pontos validados', key: 'pontos_total', width: 16 },
      { header: 'Pontos sem homologacao', key: 'pontos_base', width: 22 },
      { header: 'Pontos pendentes', key: 'pontos_pendentes', width: 16 },
      { header: 'Dias uteis', key: 'dias_uteis', width: 11 },
      { header: 'Dias de ausencia', key: 'dias_ausencia', width: 16 },
      { header: 'Dias efetivos', key: 'dias_efetivos', width: 13 },
      { header: 'Media', key: 'media', width: 10 },
      { header: 'Referencia do grupo', key: 'referencia', width: 19 },
      { header: 'Atingimento', key: 'atingimento', width: 13 },
      { header: 'Faixa', key: 'faixa', width: 22 },
      { header: 'Nivel 1', key: 'n1', width: 8 },
      { header: 'Nivel 2', key: 'n2', width: 8 },
      { header: 'Nivel 3', key: 'n3', width: 8 },
      { header: 'Nivel 4', key: 'n4', width: 8 },
      { header: 'Taxa de correcao', key: 'taxa_correcao', width: 17 },
    ];
    estilizarCabecalho(aba);
    ...
    for (const servidor of apuracao.servidores) {
      aba.addRow({
        ...servidor,
        situacao: servidor.situacao === 'APURADO' ? 'Apurado' : 'Sem apuracao',
        faixa: servidor.faixa ? ROTULO_DA_FAIXA[servidor.faixa] : 'Sem referencia',
    ...
    abaResumo.addRows([
      { indicador: 'Competencia', valor: rotularCompetencia(competencia) },
      { indicador: 'Dias uteis do mes', valor: apuracao.dias_uteis },
      { indicador: 'Media do setor (oficial)', valor: resumo.media_oficial },
      { indicador: 'Media do setor (contraprova)', valor: resumo.media_contraprova },
      { indicador: 'Total de pontos', valor: resumo.total_pontos },
      { indicador: 'Total de dias efetivos', valor: resumo.total_dias_efetivos },
      { indicador: 'Processos distintos concluidos', valor: resumo.processos_distintos },
      { indicador: 'Servidores apurados', valor: resumo.servidores_apurados },
      { indicador: 'Servidores sem apuracao', valor: resumo.servidores_sem_apuracao },
    ]);
