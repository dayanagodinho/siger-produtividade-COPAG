...
- **Aviso à chefia a cada furo**: toda mudança (turno, afastamento, feriado, regra) é comparada antes e depois; se abriu faixa sem ninguém presencial, quem mudou vê o alerta na hora e a chefia recebe um aviso no menu; quando o dia volta a ficar coberto o aviso se resolve sozinho, e se o furo mudar de horário o texto acompanha ("Avisos", com contador) e, se o Railway tiver `RESEND_API_KEY` e `EMAIL_REMETENTE`, um e-mail para cada chefia cujo login é um e-mail.
